...
int ysize = 720;
int type_int;             // 当前方块的类型
const int total = 7;     // 方块总类型数
vec4 color;              // 方块的颜色
int counter = 0;        // 重绘计数器，延缓自动下降速度

// 一个二维数组表示所有可能出现的方块和方向。

vec2 allRotationshape[7][4][4] =
{ {{vec2(-1,-1),vec2(0,-1),vec2(-1,0),vec2(0,0)},//"O"
...
{{vec2(-1,-1),vec2(0, 0), vec2(-1,0), vec2(1, 0)},	//"L"
 {vec2(0,-1), vec2(1, -1),vec2(0, 0),vec2(0, 1)},   
 {vec2(-1,0), vec2(0, 0), vec2(1,  0),vec2(1, 1)},   
 {vec2(0, -1),vec2(0, 0),vec2(-1,1), vec2(0, 1)}},

{{vec2(1,-1),vec2(0, 0), vec2(-1,0), vec2(1, 0)},	//   "J"
 { vec2(0,-1), vec2(0, 0), vec2(0, 1),vec2(1, 1)},   
 {vec2(-1,0), vec2(0, 0), vec2(1,  0),vec2(-1, 1)},   
 {vec2(-1,-1), vec2(0, -1), vec2(0, 0), vec2(0, 1)}},

 {{vec2(0, -1), vec2(-1,0), vec2(1, 0), vec2(0,0)},	//   "T"
 {vec2(0, -1), vec2(0, 0),vec2(1, 0), vec2(0,1)},   
 {vec2(1, 0), vec2(-1,0), vec2(0, 0), vec2(0, 1)},   
...
vec4 newcolor = vec4(0.5, 0.5, 0.25, 1);
vec4 colors[total] = { red, newcolor, yellow, green, blue, cyan, purple };
...
void newtile()
{
	...
	for (int i = 0; i < 4; i++)
	{
		tile[i] = allRotationshape[type_int][rotation][i];
	}

	updatetile();

	// 给新方块赋上颜色
	color = colors[type_int];
	vec4 newcolours[24];
	for (int i = 0; i < 24; i++)
		newcolours[i] = color;
...
void rotate()
{
	...
	// 检查当前旋转之后的位置的有效性
	if (checkvalid((allRotationshape[type_int][nextrotation][0]) + tilepos)
		&& checkvalid((allRotationshape[type_int][nextrotation][1]) + tilepos)
		&& checkvalid((allRotationshape[type_int][nextrotation][2]) + tilepos)
		&& checkvalid((allRotationshape[type_int][nextrotation][3]) + tilepos))
	{
...
void removeRow(int row)
{
	for (int i = 0; i < 10; i++)
	{
		...
		{
			changecellcolour(vec2(i, j), boardcolours[6 * (10 * (j + 1) + i)]);
...
void checkAllFull()
{
	vec2 newtilepos[4];
	for (int i = 0; i < 4; i++)
		newtilepos[i] = tile[i] + tilepos;
	if (checklegal(newtilepos[0])
		&& checklegal(newtilepos[1])
		&& checklegal(newtilepos[2])
		&& checklegal(newtilepos[3]))
	{ return; }
	gameover = true;
}
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("2018152062_徐明尧_期中大作业");
	glewInit();
	init();
